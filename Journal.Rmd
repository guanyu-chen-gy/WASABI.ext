...
## Warning: 
  ...
  here we set maxNclusters = 10, maxZealousAttempts = 1000.
  
## Salso
By running WASABI.ext step by step, the problem comes from R:#781 where the "while loop" break down. Later, find out that problem comes from salso::salso function with "loss = binder()" missing.
...
$$
## Test omARI.Rcpp function. Aug 01
1. Check the accuracy of the function.
```{r }
Rcpp::sourceCpp("src/omARI_Rcpp.cpp")
cls1 <- matrix(sample(0:3,25,replace = TRUE), nrow = 1)
cls2 <- matrix(sample(0:6,25,replace = TRUE), nrow = 1)
salso_loss <- salso::binder(cls1,cls2)
print(salso_loss)
