    ...
    if (counts[l] == 1) {
      ...
      if (swap_countone) {
        tmp <- test_change(
          ...
          part.evi_new, Ks.draw, Ks.part_new, loss = loss, a = a
        )
...
check_equal_particles <- function(vi.part.new, L,
  ...
  while (length(part.ind.eq) > 0) {
    ...
    if (loss == "VI"){
      ...
      vi.inds <- omARI_Rcpp(cls.draw_relab[all.inds, ], matrix(part_new_relab[l, ], nrow = 1), Ks.draw[all.inds], Ks.part_new[l])
    }
